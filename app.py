...
def add_produto(form: ProdutoSchema):
    ...
    logger.debug(f"Adicionando produto de nome: '{produto.produto}'")
    try:
        ...
        session.commit()
        logger.debug(f"Adicionado produto de nome: '{produto.produto}'")
        return apresenta_produto(produto), 200

    except IntegrityError as e:
        # como a duplicidade do nome é a provável razão do IntegrityError
        error_msg = "Produto de mesmo nome já salvo na base :/"
        logger.warning(f"Erro ao adicionar produto '{produto.produto}', {error_msg}")
...
def get_produto(query: ProdutoBuscaIdSchema):
    """Faz a busca por um Produto a partir do id do produto inserido
    ...
    if not produto:
        ...
        logger.debug(f"Produto encontrado: '{produto.id}'")
        # retorna a representação de produto
        return apresenta_produto(produto), 200
...
def del_produto(query: ProdutoBuscaIdSchema):
    ...
    if count:
        ...
        return {"mesage": "Produto removido", "produto": produto_id}
    else:
        # se o produto não foi encontrado
        error_msg = "Produto não encontrado na base :/"
...
def update_produto(query:ProdutoUpdateSchema, form: ProdutoSchema):
    """Atualiza um Produto existente na base de dados

    Retorna uma representação dos produtos e comentários associados após a atualização.
    """

    produto_id = query.id
    logger.debug(f"Atualizando produto de nome: '{produto_id}'")
    session = Session()
    produto = session.query(Produto).filter(Produto.id == produto_id).first()
    try:
        # criando conexão com a base
        ...
        # Atualize os campos relevantes do produto com os dados do formulário
        produto.produto = form.produto
        produto.tamanho = form.tamanho
        ...
        logger.warning(f"Erro ao atualizar o produto '{produto_id}', {error_msg}")
